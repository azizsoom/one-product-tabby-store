...
export default async function PaymentCancelPage({ searchParams }: { searchParams: Promise<{ order_id?: string }> }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-stone-50 p-6 text-stone-950" dir="rtl">
      <section className="w-full max-w-xl rounded-3xl bg-white p-8 text-center shadow-sm ring-1 ring-stone-200">
        <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-amber-100 text-4xl text-amber-700">!</div>
        <h1 className="mt-6 text-3xl font-black">تم إلغاء عملية الدفع</h1>
        <p className="mt-3 text-stone-600">لم يكتمل الدفع. يمكنك الرجوع للمتجر وإعادة المحاولة.</p>
        {orderId ? (
