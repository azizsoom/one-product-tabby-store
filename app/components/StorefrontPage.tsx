...
export default function StorefrontPage() {
  ...
  const preview = useMemo(() => calculateCartOrder(cart, appliedDiscount, fulfillmentType), [cart, appliedDiscount, fulfillmentType]);
  ...
  async function startTabbyCheckout() {
    ...
    if (fulfillmentType === 'shipping' && (!shippingCity.trim() || !shippingAddress.trim())) {
      setSaving(false);
      setMessage('فضلاً أدخل المدينة والعنوان التفصيلي للشحن.');
      return;
    }
      ...
      <section className="mx-auto max-w-7xl px-4 py-8">
        ...
        <section className="mb-5 rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-stone-200">
          <div className="grid gap-3 md:grid-cols-[1fr_220px_auto] md:items-center">
            ...
            <div className="rounded-2xl bg-stone-50 px-4 py-3 text-sm font-bold text-stone-700 ring-1 ring-stone-200">
              {filteredProducts.length} ظاهر / {availableCount} متوفر
            </div>
          </div>
        </section>
        ...
        {filteredProducts.length === 0 ? <div className="rounded-[2rem] bg-white p-8 text-center text-stone-600 shadow-sm ring-1 ring-stone-200">لا توجد منتجات مطابقة للبحث.</div> : (
              ...
              return (
                <div key={product.id} className="group overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-stone-200 transition hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-3 rounded-2xl bg-stone-50 p-3 text-center"><p className="text-xs font-bold text-orange-500">أفضل سعر</p><p className="text-2xl font-black text-stone-900">{Number(product.price_before_vat || 0).toFixed(0)}</p><p className="text-xs text-stone-500">ريال قبل الضريبة</p></div>
      ...
      {cartOpen ? (
        <div className="fixed inset-0 z-50">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-full max-w-md flex-col bg-white shadow-2xl md:rounded-r-[2rem]" dir="rtl">
            <div className="flex items-center justify-between border-b border-stone-200 p-4"><div><h2 className="text-2xl font-black">سلة المشتريات</h2><p className="text-sm text-stone-500">{totalQuantity} منتج في السلة</p></div><button onClick={() => setCartOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-2xl bg-stone-100 text-stone-700"><X size={22} /></button></div>
...
function calculateCartOrder(cart: CartItem[], discount: DiscountCode | null, fulfillmentType: 'shipping' | 'pickup') {
  ...
  const shippingAmount = fulfillmentType === 'pickup' ? 0 : roundMoney(cart.length === 0 ? 0 : Math.max(...cart.map((item) => Number(item.product.shipping_amount || 0))));
