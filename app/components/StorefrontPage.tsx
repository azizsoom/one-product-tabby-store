...
type CityOption = { label: string; value: string; region: string };
...
const SHIPPING_CITY_OPTIONS: CityOption[] = [
  { label: 'الرياض', value: 'Riyadh', region: 'الوسطى' },
  { label: 'الخرج', value: 'Al Kharj', region: 'الوسطى' },
  { label: 'الدرعية', value: 'Diriyah', region: 'الوسطى' },
  { label: 'الدوادمي', value: 'Dawadmi', region: 'الوسطى' },
  { label: 'المجمعة', value: 'Al Majmaah', region: 'الوسطى' },
  { label: 'الزلفي', value: 'Zulfi', region: 'الوسطى' },
  { label: 'وادي الدواسر', value: 'Wadi ad-Dawasir', region: 'الوسطى' },
  ...
  { label: 'رابغ', value: 'Rabigh', region: 'الغربية' },
  { label: 'بحرة', value: 'Bahrah', region: 'الغربية' },
  { label: 'القنفذة', value: 'Al Qunfudhah', region: 'الغربية' },
  { label: 'الليث', value: 'Al Lith', region: 'الغربية' },
  { label: 'العلا', value: 'Al Ula', region: 'الغربية' },
  { label: 'الدمام', value: 'Dammam', region: 'الشرقية' },
  { label: 'الخبر', value: 'Khobar', region: 'الشرقية' },
  { label: 'الظهران', value: 'Dhahran', region: 'الشرقية' },
  { label: 'القطيف', value: 'Qatif', region: 'الشرقية' },
  { label: 'الجبيل', value: 'Jubail', region: 'الشرقية' },
  { label: 'الأحساء', value: 'Al Ahsa', region: 'الشرقية' },
  { label: 'الهفوف', value: 'Hofuf', region: 'الشرقية' },
  { label: 'حفر الباطن', value: 'Hafar Al-Batin', region: 'الشرقية' },
  { label: 'الخفجي', value: 'Khafji', region: 'الشرقية' },
  ...
  { label: 'رفحاء', value: 'Rafha', region: 'الشمال' },
  { label: 'طريف', value: 'Turaif', region: 'الشمال' },
  { label: 'ضباء', value: 'Duba', region: 'الشمال' },
  { label: 'أملج', value: 'Umluj', region: 'الشمال' },
  ...
  { label: 'بيشة', value: 'Bisha', region: 'الجنوب' },
  { label: 'الباحة', value: 'Al Bahah', region: 'الجنوب' },
  { label: 'بلجرشي', value: 'Baljurashi', region: 'الجنوب' },
];

const CITY_GROUPS = Array.from(new Set(SHIPPING_CITY_OPTIONS.map((city) => city.region)));
...
export default function StorefrontPage() {
  ...
  useEffect(() => {
    ...
    setShippingOptions([]);
    setShippingMessage('');
  }, [shippingCity, fulfillmentType, cart.length]);
  ...
  const selectedCityLabel = getCityLabel(shippingCity);
  ...
  async function applyDiscountCode() {
    ...
    setDiscountMessage(`تم تطبيق كود الخصم ${discount.code}.`);
  }

  function removeDiscount() {
    setAppliedDiscount(null);
    setDiscountCode('');
    setDiscountMessage('تم حذف كود الخصم.');
  }
  ...
  async function fetchShippingOptions() {
    ...
    if (!shippingCity.trim()) return setShippingMessage('اختر مدينة الشحن أولاً.');
    ...
    setShippingMessage(data.warning || `تم جلب ${options.length} خيار شحن إلى ${selectedCityLabel}.`);
  }
  ...
  async function startTabbyCheckout() {
    ...
    if (fulfillmentType === 'shipping' && (!shippingCity.trim() || !shippingAddress.trim())) {
      setSaving(false);
      setMessage('فضلاً اختر المدينة وأدخل العنوان التفصيلي للشحن.');
      return;
    }
    if (fulfillmentType === 'shipping' && !selectedShipping) {
      setSaving(false);
      setMessage('فضلاً احسب الشحن واختر شركة الشحن قبل الدفع.');
      return;
    }
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-stone-50 text-stone-950" dir="rtl">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8">
        <div className="mb-8 rounded-[2rem] bg-stone-950 p-6 text-white shadow-sm md:p-8">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-semibold text-emerald-100"><ShieldCheck size={18} /> متجر متعدد المنتجات مربوط بتابي</div>
          <h2 className="mt-5 text-3xl font-black leading-tight md:text-5xl">اختر منتجاتك واجمعها في طلب واحد</h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-stone-200 md:text-base">ابحث عن المنتج، رتب النتائج، أضف للسلة، ثم أكمل الدفع من النافذة المنبثقة.</p>
        </div>

        <section className="mb-5 rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-stone-200">
          <div className="grid gap-3 md:grid-cols-[1fr_220px_auto] md:items-center">
            <label className="relative block">
              <Search className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-stone-400" size={18} />
              <input className="input pr-11" placeholder="ابحث باسم المنتج أو الوصف" value={search} onChange={(e) => setSearch(e.target.value)} />
            </label>
            ...
            <div className="rounded-2xl bg-stone-50 px-4 py-3 text-sm font-bold text-stone-700 ring-1 ring-stone-200">{filteredProducts.length} ظاهر / {availableCount} متوفر</div>
          </div>
        </section>
        ...
        {filteredProducts.length === 0 ? (
          <div className="rounded-[2rem] bg-white p-8 text-center text-stone-600 shadow-sm ring-1 ring-stone-200">لا توجد منتجات مطابقة للبحث.</div>
        ) : (
              ...
              return (
                <div key={product.id} className="group overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-stone-200 transition hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div className="p-3">
                    ...
                    <button onClick={() => addToCart(product)} disabled={isOut} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-3 py-3 text-sm font-black text-white transition hover:bg-emerald-700 disabled:bg-stone-300"><ShoppingBag size={16} /> إضافة للسلة</button>
                  </div>
                </div>
              );
            })}
          ...
          <aside className="absolute left-0 top-0 flex h-full w-full max-w-md flex-col bg-white shadow-2xl md:rounded-r-[2rem]" dir="rtl">
            ...
            <div className="flex-1 overflow-y-auto p-4">
                  ...
                  {cart.map((item) => (
                    <div key={item.product.id} className="rounded-3xl border border-stone-200 p-3">
                      <div className="flex gap-3">
                        <div className="h-20 w-20 shrink-0 overflow-hidden rounded-2xl bg-stone-100">{item.product.image_url ? <img src={item.product.image_url} alt={item.product.name} className="h-full w-full object-cover" /> : null}</div>
              ...
              <div className="mt-4 rounded-3xl bg-stone-50 p-4 ring-1 ring-stone-200">
                <h3 className="font-black">كود الخصم</h3>
                <div className="mt-3 flex gap-2">
                  <input className="input" placeholder="مثال: SAVE10" value={discountCode} onChange={(e) => setDiscountCode(e.target.value)} />
                  <button onClick={applyDiscountCode} className="rounded-2xl bg-stone-950 px-4 font-bold text-white">تطبيق</button>
                </div>
                {appliedDiscount ? <button onClick={removeDiscount} className="mt-2 text-xs font-bold text-red-600">إزالة الخصم</button> : null}
                {discountMessage ? <p className="mt-2 text-xs text-stone-600">{discountMessage}</p> : null}
              </div>
              ...
              <div className="mt-4 rounded-3xl bg-stone-50 p-4 ring-1 ring-stone-200">
                <h3 className="font-black">الشحن والاستلام</h3>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button onClick={() => setFulfillmentType('shipping')} className={`rounded-2xl px-4 py-3 text-sm font-bold ring-1 ${fulfillmentType === 'shipping' ? 'bg-emerald-600 text-white ring-emerald-600' : 'bg-white text-stone-700 ring-stone-200'}`}>شحن</button>
                  <button onClick={() => setFulfillmentType('pickup')} className={`rounded-2xl px-4 py-3 text-sm font-bold ring-1 ${fulfillmentType === 'pickup' ? 'bg-emerald-600 text-white ring-emerald-600' : 'bg-white text-stone-700 ring-stone-200'}`}>استلام من الفرع</button>
                </div>
                {fulfillmentType === 'shipping' ? (
                  <div className="mt-3 grid gap-3">
                    <select className="input" value={shippingCity} onChange={(e) => setShippingCity(e.target.value)} aria-label="مدينة الشحن">
                      {CITY_GROUPS.map((region) => (
                        <optgroup key={region} label={region}>
                          {SHIPPING_CITY_OPTIONS.filter((city) => city.region === region).map((city) => (
                            <option key={city.value} value={city.value}>{city.label}</option>
                          ))}
                        </optgroup>
                      ))}
                    </select>
                    <input className="input" placeholder="الحي" value={shippingDistrict} onChange={(e) => setShippingDistrict(e.target.value)} />
                    <textarea className="input min-h-20" placeholder="العنوان التفصيلي" value={shippingAddress} onChange={(e) => setShippingAddress(e.target.value)} />
                    <textarea className="input min-h-16" placeholder="ملاحظات إضافية للطلب اختياري" value={shippingNotes} onChange={(e) => setShippingNotes(e.target.value)} />
                    <button onClick={fetchShippingOptions} disabled={shippingLoading || cart.length === 0} className="rounded-2xl bg-sky-600 px-4 py-3 font-bold text-white disabled:opacity-50">{shippingLoading ? 'جاري حساب الشحن...' : 'حساب خيارات الشحن'}</button>
                    {shippingMessage ? <p className="text-xs text-stone-600">{shippingMessage}</p> : null}
                    {shippingOptions.length > 0 ? (
                      <div className="grid gap-2">
                        {shippingOptions.map((option) => (
                          <button key={option.id} onClick={() => setSelectedShipping(option)} className={`rounded-2xl p-3 text-right ring-1 ${selectedShipping?.id === option.id ? 'bg-emerald-50 ring-emerald-500' : 'bg-white ring-stone-200'}`}>
                            <div className="flex items-center justify-between gap-2"><strong>{option.company}</strong><strong>{Number(option.price || 0).toFixed(2)} {option.currency || 'SAR'}</strong></div>
                            <p className="mt-1 text-xs text-stone-500">{option.service}{option.eta ? ` - ${option.eta}` : ''}</p>
                          </button>
                        ))}
                      </div>
                    ) : null}
                  ...
                  <p className="mt-3 rounded-2xl bg-white p-3 text-sm text-stone-600 ring-1 ring-stone-200">سيتم تجهيز الطلب للاستلام من الفرع، ولن يتم احتساب الشحن.</p>
                )}
              </div>
              ...
              <div className="mt-4 rounded-3xl bg-white p-4 ring-1 ring-stone-200">
                ...
                <div className="mt-3 space-y-2 text-sm">
                  ...
                  <Row label={appliedDiscount ? `خصم ${appliedDiscount.code}` : 'الخصم'} value={`-${preview.discountAmount.toFixed(2)} ريال`} />
                  ...
                  <Row label={selectedShipping ? `الشحن - ${selectedShipping.company}` : 'الشحن'} value={preview.shippingAmount === 0 ? 'مجاني' : `${preview.shippingAmount.toFixed(2)} ريال`} />
                  <div className="mt-3 flex items-center justify-between rounded-2xl bg-stone-950 px-4 py-3 text-white"><span>الإجمالي</span><strong className="text-xl">{preview.totalAmount.toFixed(2)} ريال</strong></div>
                </div>
            ...
            <div className="border-t border-stone-200 bg-white p-4">
              <button onClick={startTabbyCheckout} disabled={saving || cart.length === 0} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-6 py-4 text-lg font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-50"><ShoppingBag size={20} /> {saving ? 'جاري تحويلك إلى تابي...' : 'الدفع عبر تابي'}</button>
...
function getCityLabel(value: string) {
  return SHIPPING_CITY_OPTIONS.find((city) => city.value === value)?.label || value;
}
