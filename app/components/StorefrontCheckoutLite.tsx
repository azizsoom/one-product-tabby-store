...
export default function StorefrontCheckoutLite() {
  ...
  function animateToCart(el?:HTMLElement|null){
    ...
    window.setTimeout(()=>setFlyItems(items=>items.filter(i=>i.id!==id)),850);
  }
  ...
  return <main dir="rtl" className="min-h-screen bg-stone-50 text-stone-950">
    ...
    {flyItems.map(item=><span key={item.id} className="pointer-events-none fixed z-[80] text-3xl drop-shadow-lg" style={{left:item.fromX,top:item.fromY,transform:'translate(-50%,-50%)',animation:`fly-cart-${item.id} 850ms cubic-bezier(.22,1,.36,1) forwards`}}>{item.emoji}<style>{`@keyframes fly-cart-${item.id}{0%{left:${item.fromX}px;top:${item.fromY}px;opacity:1;transform:translate(-50%,-50%) scale(1)}70%{opacity:1;transform:translate(-50%,-50%) scale(1.25)}100%{left:${item.toX}px;top:${item.toY}px;opacity:0;transform:translate(-50%,-50%) scale(.35)}}`}</style></span>)}
